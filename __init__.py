...
class ThicketPropGroup(PropertyGroup):
    ...
    batch_use_lod: BoolProperty(name="Show Batch LOD",
                                description="Change LOD for selected plants.",
                                default=False)
...
class THICKET_PT_plant_properties(Panel):
    ...
    def draw_props(self, layout, plant, tp, batch=False):
    ...
    def draw_batch_props(self, layout, bp):
        """Draw Laubwerk standard properties UI"""
        layout.prop(bp, "model")
        layout.prop(bp, "qualifier")
    ...
    def draw(self, context):
        ...
        if not batch or col.enabled:
            self.draw_props(col, plant, tp, batch)

        if not batch and thicket_ui_mode == 'VIEW':
